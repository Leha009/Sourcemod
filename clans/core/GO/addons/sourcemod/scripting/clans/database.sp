...
void DB_CreateClanCallback(Handle owner, Handle hndl, const char[] error, DataPack data)
{
	...
	else
	{
		...
		Call_PushCell(g_iClientData[leaderid][CLIENT_CLANID]);
...
{
	char query[200];
	FormatEx(query, sizeof(query), "UPDATE `clans_table` SET `clan_coins` = `clan_coins`+'%d' WHERE `clan_id` = '%d'", amountToAdd, clanid);
...
{
	char query[200];
	FormatEx(query, sizeof(query), "UPDATE `clans_table` SET `clan_kills` = `clan_kills`+'%d' WHERE `clan_id` = '%d'", amountToAdd, clanid);
...
{
	char query[200];		
	FormatEx(query, sizeof(query), "UPDATE `clans_table` SET `clan_deaths` = `clan_deaths`+'%d' WHERE `clan_id` = '%d'", amountToAdd, clanid);
...
void DB_CreateClient(int client)
{
	...
	Call_PushCell(g_iClientData[client][CLIENT_CLANID]);
	Call_Finish();
}
 ...
 */
void DB_CreateClientByData(const char[] name, const char[] auth, int clanid, int role)
{
	SQL_LockDatabase(g_hClansDB);
	char query[400];
	FormatEx(query, sizeof(query), "SELECT MAX(`player_id`) FROM `players_table`");
